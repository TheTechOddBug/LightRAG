...
- Every public SDK graph mutation (`insert_custom_kg` and entity/relation create, edit, delete, and merge) reserves the same workspace `busy` writer slot through `LightRAG._graph_mutation_slot`; per-entity locks alone cannot protect whole-namespace NetworkX commits. Internal `utils_graph` mutation helpers require their caller to provide this serialization.
